...
def login_user(username, password):
    ...
    if user and bcrypt.checkpw(password.encode("utf-8"), user[1].encode("utf-8")):
        user_id, _, role = user
        messagebox.showinfo("Σύνδεση", f"Καλώς ήρθες, {username}! Ρόλος: {role}")

        try:
            if role == 'admin':
                subprocess.Popen([sys.executable, "admin_dashboard.py", username])
            elif role == 'user':
                subprocess.Popen([sys.executable, "user_dashboard.py", str(user_id)])
            else:
                messagebox.showerror("Σφάλμα", f"Μη αναγνωρισμένος ρόλος: {role}")
                return

            tk._default_root.destroy()  # Κλείσιμο του παραθύρου login
        except Exception as e:
            messagebox.showerror("Σφάλμα", f"Αποτυχία εκκίνησης dashboard:\n{e}")
    else:
